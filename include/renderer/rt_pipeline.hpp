...
public:
    ...
    RTPipeline(vk::Device & device, VmaAllocator & allocator, vk::detail::DispatchLoaderDynamic& dl, const std::vector<vk::DescriptorSetLayoutBinding>& bindings, 
        ...
        if (modules.size() != 3) {
            throw std::runtime_error("Not enough shader modules provided for pipeline creation");
        }
        ...
        vk::PipelineShaderStageCreateInfo shader_stages[3] = {
            {vk::PipelineShaderStageCreateFlags(), vk::ShaderStageFlagBits::eRaygenKHR, modules[0], "main"},
            {vk::PipelineShaderStageCreateFlags(), vk::ShaderStageFlagBits::eMissKHR, modules[1], "main"},
            {vk::PipelineShaderStageCreateFlags(), vk::ShaderStageFlagBits::eClosestHitKHR, modules[2], "main"}
        };
