...
class WindowSystemGLFW final: public WindowSystem {
	private:
		...
		std::unordered_map<WindowHandle, GLFWwindow*, HandleHasher<WindowHandle>> windows;
	...
	size_t num_windows() const override {
		return windows.size();
	}
	...
	void destroy_window(WindowHandle window) override {
		...
		WindowUserData* wud = static_cast<WindowUserData*>(glfwGetWindowUserPointer(windows[window]));
		delete wud;
	...
	GLFWwindow * get(WindowHandle window) {
		return windows[window];
	}

	static void window_focus_callback(GLFWwindow* window, int focused) {
		auto wud = static_cast<WindowUserData*>(glfwGetWindowUserPointer(window));
		WindowSystemGLFW* window_system = dynamic_cast<WindowSystemGLFW*>(wud->window_system);
	...
	void register_create_window_callback(std::function<void(WindowHandle)> callback) override {
