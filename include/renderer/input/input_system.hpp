...
private:
	...
	std::unordered_map<input::MouseButton, std::vector<input::MouseButtonBinding>> mouse_button_bindings;
	std::unordered_map<std::string, input::ActionHandle> action_names;
	std::unordered_map<input::ActionHandle, input::Action, HandleHasher<input::ActionHandle>> actions;
...
public:
	InputSystem(WindowSystem* window_system,  KeyboardInterface* keyboard_interface,  MouseInterface* mouse_interface);
	~InputSystem();
	void reload_controls();
	bool update();
	void create_key_action_binding(std::string action_name, input::Key key, bool invert = false);
	void create_mouse_button_action_binding(std::string action_name, input::MouseButton mouse_button, bool invert = false);
