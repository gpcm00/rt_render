...
public:
	MouseGLFW(WindowSystem* window_system): position_x(0), position_y(0) {
		...
		window_system->register_create_window_callback([&] (WindowHandle handle) { onCreateWindow(handle); });
	...
	virtual const std::vector<input::MouseEvent>& get_mouse_events() override {
		...
		for (const auto & last_event: previous_mouse_events) {
			if (last_event.button_state == input::ButtonState::Pressed || last_event.button_state == input::ButtonState::Held ) {
				// find
				auto it = std::find_if(mouse_events.cbegin(), mouse_events.cend(),
				[&] (const input::MouseEvent & event) { return event.button == last_event.button;});

				if (it == mouse_events.end()) {
					mouse_events.push_back({last_event.button, input::ButtonState::Held});
				}
			}
		}
		return mouse_events;
	...
	static void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
		auto window_user_data = static_cast<WindowUserData*>(glfwGetWindowUserPointer(window));
		auto mouse_interface = dynamic_cast<MouseGLFW*>(window_user_data->mouse_interface);
