...
class RTCamera {
  public:
    ...
    void set_position(const glm::vec3 &pos) { position = glm::vec4(pos, 0.0f); }

    void set_direction(const glm::vec3 &dir) {
        direction = glm::vec4(dir, 0.0f);
    }

    void set_up(const glm::vec3 &up_vec) { up = glm::vec4(up_vec, 0.0f); }

    void set_right(const glm::vec3 &right_vec) {
        right = glm::vec4(right_vec, 0.0f);
    }

    void set_fov(float fov_degrees) { fov = glm::radians(fov_degrees / 2.0f); }
