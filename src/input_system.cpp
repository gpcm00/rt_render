...
void InputSystem::evaluate_key_events() {
    ...
    for (auto &key_event : key_events) {
        ...
        for (auto &binding : key_bindings[key_event.key]) {
            ...
            float axis = 0.0f;
            if ((key_event.button_state == input::ButtonState::Pressed ||
                 key_event.button_state == input::ButtonState::Held)) {
                if (binding.inverted) {
                    axis = -1.0f;
                } else {
                    axis = 1.0f;
            ...
            if (key_event.button_state == input::ButtonState::Pressed &&
                actions.find(binding.action) != actions.end() &&
                (actions[binding.action].button_state ==
...
void InputSystem::evaluate_mouse_events() {
    ...
    for (auto &mouse_event : mouse_events) {
        ...
        for (auto &binding : mouse_button_bindings[mouse_event.button]) {
            ...
            float axis = 0.0f;
            if (mouse_event.button_state == input::ButtonState::Pressed ||
                mouse_event.button_state == input::ButtonState::Held) {
                if (binding.inverted) {
                    axis = -1.0f;
                } else {
                    axis = 1.0f;
...
InputSystem::InputSystem(WindowSystem *window_system,
    ...
    : window_system(window_system), keyboard_interface(keyboard_interface),
      mouse_interface(mouse_interface) {
    create_key_action_binding("Exit", input::Key::Escape);
    create_mouse_button_action_binding("Mouse Action",
                                       input::MouseButton::RightMouse);
...
void InputSystem::reload_controls() { actions.clear(); }
...
input::MousePosition InputSystem::get_mouse_position() {
    return input::MousePosition(mouse_interface->get_x(),
                                mouse_interface->get_y());
}
...
bool InputSystem::update() {
    ...
    if (get_button_state("Exit") == input::ButtonState::Pressed ||
        get_button_state("Exit") == input::ButtonState::Held) {
        return true;
    }
    return false;
}
void InputSystem::create_key_action_binding(std::string action_name,
                                            input::Key key, bool invert) {
...
void InputSystem::create_mouse_button_action_binding(
    std::string action_name, input::MouseButton mouse_button, bool invert) {
    auto action = get_action(action_name);
    mouse_button_bindings[mouse_button].push_back(
        input::MouseButtonBinding{mouse_button, action, invert});
}
