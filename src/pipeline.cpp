...
static vk::RayTracingShaderGroupCreateInfoKHR
populate_group(vk::ShaderStageFlagBits stage_flag, uint32_t index) {
...
void Pipeline::push_module(std::string file_path,
                           vk::ShaderStageFlagBits flag) {
    PipelineModules module = {.file_path = file_path,
...
void Pipeline::add_binding(vk::DescriptorType type, vk::ShaderStageFlags flags,
                           uint32_t count) {
...
void Pipeline::create_set() {
    ...
    if (device.createDescriptorSetLayout(&info, nullptr, &set) !=
        vk::Result::eSuccess) {
...
void Pipeline::create_rt_pipeline(uint32_t ray_depth) {
    ...
    for (auto &module : modules) {
        ...
        vk::RayTracingShaderGroupCreateInfoKHR group =
            populate_group(module.stage_flag, nstage);
    ...
    constants.stageFlags = vk::ShaderStageFlagBits::eRaygenKHR |
    ...
    auto ret = device.createRayTracingPipelineKHR(nullptr, nullptr,
                                                  pipeline_info, nullptr, dl);
