...
Swapchain::Swapchain(vk::PhysicalDevice &physical_device, vk::Device &device,
    ...
    const auto surface_format = vk::SurfaceFormatKHR(
    ...
    // we also have to decide how many images we would like to have in the swap
    // chain (take an extra b/c sometimes have to wait for driver)
    ...
    std::cout << "Graphics family index: " << graphics_family_index
              << std::endl;
    std::cout << "Present family index: " << present_family_index << std::endl;

    uint32_t queueFamilyIndices[2] = {
        static_cast<uint32_t>(graphics_family_index),
        static_cast<uint32_t>(present_family_index)};
    ...
    create_info.setPreTransform(
        capabilities.currentTransform); // we want the current transform (don't
                                        // want to change/flip/rotate
    create_info.setCompositeAlpha(
        vk::CompositeAlphaFlagBitsKHR::eOpaque); // no blending with other
                                                 // windows in window system
                                                 // (ignore alpha channel)
    create_info.setPresentMode(present_mode);
    create_info.setClipped(
        VK_TRUE); // ignore obscured pixels when window is in front
    create_info.setOldSwapchain(nullptr);

    // before creating swap chain, query for WSI support ( validation complains
    // without this, I don't even know what this means)
    if (physical_device.getSurfaceSupportKHR(queueFamilyIndices[0], surface) !=
        VK_TRUE) {
        throw std::runtime_error(
            "This physical device does not include WSI support!");
    }
    ...
    for (size_t i = 0; i < images.size(); i++) {
        vk::ImageViewCreateInfo iv_create_info(
            {}, images[i], vk::ImageViewType::e2D, format,
...
unsigned int Swapchain::get_num_images() { return image_views.size(); }
uint32_t Swapchain::get_width() { return extent.width; }
uint32_t Swapchain::get_height() { return extent.height; }
vk::Extent2D &Swapchain::get_extent() { return extent; }
vk::Format &Swapchain::get_format() { return format; }
vk::Image &Swapchain::get_image(unsigned int index) { return images.at(index); }
...
vk::SwapchainKHR &Swapchain::get_swapchain() { return swapchain.get(); }
...
void Swapchain::create_framebuffers(vk::UniqueDevice &device,
                                    const vk::RenderPass &render_pass,
                                    vk::ImageView depth_image_view) {
    const auto num_images = get_num_images();
    framebuffers.resize(num_images);
    for (unsigned int i = 0; i < num_images; i++) {
        std::vector<vk::ImageView> attachments = {get_image_view(i),
                                                  depth_image_view};
        vk::FramebufferCreateInfo fbCI(
            {}, render_pass, static_cast<uint32_t>(attachments.size()),
            attachments.data(), get_width(), get_height(), 1);
