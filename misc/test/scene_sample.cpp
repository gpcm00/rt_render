...
const std::string MODEL_PATH =
    "glTF-Sample-Assets/Models/AntiqueCamera/glTF/AntiqueCamera.gltf";
const std::string TEXTURE_PATH =
    "glTF-Sample-Assets/Models/AntiqueCamera/glTF/camera_camera_BaseColor.png";
...
std::array<T, N + M> concat(const std::array<T, N> &a,
...
class HelloTriangleApplication {
  ...
  private:
    ...
    void createGraphicsPipeline() {
        ...
        auto attributeDescriptions = concat(Vertex::getAttributeDescriptions(),
                                            getMat4AttributeDescriptions());
    ...
    void createMeshBuffer() {
        ...
        for (auto &shape : shapes) {
            createVertexBuffer(meshes[mesh_i].vertexBuffer,
                               meshes[mesh_i].vertexBufferMemory, shape.mesh);
            createInstanceBuffer(meshes[mesh_i].instanceBuffer,
                                 meshes[mesh_i].instanceBufferMemory,
                                 &shape.transformation);
            createIndexBuffer(meshes[mesh_i].indexBuffer,
                              meshes[mesh_i].indexBufferMemory, shape.mesh);
            meshes[mesh_i].indexBufferSize =
                static_cast<uint32_t>(shape.mesh->indices.size());
            mesh_i++;
        }
    }

    void createVertexBuffer(VkBuffer &vertexBuffer,
                            VkDeviceMemory &vertexBufferMemory,
                            const Mesh *mesh) {
    ...
    void createInstanceBuffer(VkBuffer &instanceBuffer,
                              VkDeviceMemory &instanceBufferMemory,
    ...
    void createIndexBuffer(VkBuffer &indexBuffer,
                           VkDeviceMemory &indexBufferMemory,
                           const Mesh *mesh) {
        VkDeviceSize bufferSize =
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer,
        ...
        for (auto &mesh : meshes) {
            ...
            vkCmdBindVertexBuffers(commandBuffer, 0, 2, vertexBuffers,
                                   offsetsv);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.view = glm::lookAt(glm::vec3(18.0f, 18.0f, 18.0f),
                               glm::vec3(0.0f, 0.0f, 0.0f),
                               glm::vec3(0.0f, 0.0f, 1.0f));
        ubo.proj = glm::perspective(
            glm::radians(45.0f),
            swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 70.0f);
        ubo.proj[1][1] *= -1;
